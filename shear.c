...
void *thread_function(void *arg)
{
	...
	for(k = 0; k < ceil(log(L * L)) + 1; k++) {
		// prima faza : sortarea randurilor
		for(i = start; i < end; i++) {
			if (i % 2) {
				// sortez descrescator randurile impare
	 			qsort(M[i], L, sizeof(int), cmpdesc);
	 		} else {
				// sortez crescator randurile pare
	 			qsort(M[i], L, sizeof(int), cmp);
			}
		}
		pthread_barrier_wait(&barrier);

		// a doua faza : sortarea coloanelor
		for(i = start; i < end; i++) {
			for(j = 0; j < L; j++) {
				aux[j] = M[j][i];
			}
			// sortez crescator coloanele
